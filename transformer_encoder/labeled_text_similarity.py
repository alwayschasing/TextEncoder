...
from encoder_model import batch_to_device
from losses import SoftmaxLoss
from data_util import SentencesDataset, LabeledSTSDataReader
...
def main(args):
    ...
    if args.do_train == 1:
        ...
        data_reader = LabeledSTSDataReader()
        ...
        train_loss = SoftmaxLoss(model=model, sentence_embedding_dimension=reduce_output_size, num_labels=3)
    ...
    if args.do_predict == 1:
        ...
        softmax_model = SoftmaxLoss(model, 128, 3)
        softmax_model.to(model.device)
        softmax_model.load_state_dict(torch.load(os.path.join(args.init_model,"loss_model.bin")))
        data_reader = LabeledSTSDataReader()
        input_examples = data_reader.get_examples(pred_data_file, skip_head=args.skip_firstline, predict_mode=True)
        ...
        pred_dataloader.collate_fn = model.smart_batching_collate
        predict_res = []
        with torch.no_grad():
            for batch_data in tqdm(pred_dataloader, desc="Iteration", smoothing=0.05):
                features, labels = batch_to_device(batch_data, model.device)
                _, logits, softmax_out = softmax_model.forward(features, None)
                pred_label = torch.argmax(softmax_out, dim=1)
                predict_res.extend(pred_label)
        ...
        #softmax_output = softmax_model(model.encode())
        #with open(predict_output, "w", encoding="utf-8") as fp:
        #    for idx, item in enumerate(predict_res):
        #        input_example = input_examples[idx]
        #        fp.write("%s\t%s\t%f\n"%(input_example.texts[0], input_example.texts[1], item))
