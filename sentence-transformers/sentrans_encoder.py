...
class SenTransServer(threading.Thread):
    ...
    def _run(self,_,backend_sock):
        backend_sock.bind('tcp://10.153.57.105:%d' % self.port)
        self.logger.info("bind word2vec_server socket:%s:%d"%("tcp://10.53.57.105",self.port))
        while True:
            """server job""" 
            try:
                ...
                print("%s"%(text_embedding))
                self.logger.info("%s"%(text_embedding[0].tolist()))
...
def start_server():
    ...
    logger.info("build SenTransEncoder")
    listen_port = 5021
    index_vecs_file = "/search/odin/liruihong/article_data/index_data/titles_5d_utf8_retrain.bin"
